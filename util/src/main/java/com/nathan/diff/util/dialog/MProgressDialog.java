package com.nathan.diff.util.dialog;
...
public class MProgressDialog implements View.OnClickListener {
    ...
    private void timeTrigger() {
        ...
        timer.schedule(new Mytack(), date, 10000);//第一个参数为定时调用的方法，注意是一次性的，如果关闭Timer得重新实例化， 第二个参数为第一次调用的时间，第三个参数为两次调用方法的间隔毫秒数
    ...
    public void  setCancelTouchOutside(boolean flag){
        mBuilder.canceledOnTouchOutside=flag;
    }
