package com.nathan.diff.util.edit;
...
public class MClearEditText extends android.support.v7.widget.AppCompatEditText implements View.OnFocusChangeListener, TextWatcher {
    ...
    private void initCustomAttrs(Context context, AttributeSet attrs, int defStyleAttr) {
        //获取自定义属性。
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.utilMClearEditText, defStyleAttr, 0);
        //是否展示底部线
        showBottomLine = ta.getBoolean(R.styleable.utilMClearEditText_mClearEditText_showBottomLine, true);
        //底部线的颜色
        defaultBottomLineColor = ta.getColor(R.styleable.utilMClearEditText_mClearEditText_bottomLineColor, Color.parseColor("#707070"));
        //底部线的宽度dp
        bottomLineWidth = ta.getDimension(R.styleable.utilMClearEditText_mClearEditText_bottomLineWidth, dip2px(context, 2));
        //左边按钮的大小dp
        leftDrawableSize = ta.getDimension(R.styleable.utilMClearEditText_mClearEditText_leftDrawableSize, dip2px(context, 20));
        //清除功能是否可用
        disableClean = ta.getBoolean(R.styleable.utilMClearEditText_mClearEditText_disableClear, true);
        //左边按钮的大小dp
        rightDrawableSize = ta.getDimension(R.styleable.utilMClearEditText_mClearEditText_rightDrawableSize, dip2px(context, 20));
        ta.recycle();
    }
